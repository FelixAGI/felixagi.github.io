...
const payload = fs.readFileSync(path.join(__dirname, "open/image/owo-1000.fqx"), "ascii").trim();
...
assert.equal(binary.subarray(16, 16 + nameLength).toString("utf8"), "owo.jpg");
const exact = binary.subarray(16 + nameLength);
assert.equal(exact.length, fileLength);
assert.equal(fileLength, 70_277);
assert.equal(crypto.createHash("sha256").update(exact).digest("hex"), "07e4cf775b42b1ba7bc7a23ad2f9fe80f7ae41f48367e0b95fdcd845c3582e48");
